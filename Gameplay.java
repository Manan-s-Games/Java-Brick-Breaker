...
public class Gameplay extends JPanel implements KeyListener, ActionListener {
  ...
  private int ballxdir = (int)Math.floor(Math.random()*(5-(-5)+1)+(-5));
  private int ballydir = (int)Math.floor(Math.random()*(5-(-5)+1)+(-5));
  ...
  public void keyPressed(KeyEvent e) {
    ...
    if(e.getKeyCode() == KeyEvent.VK_ENTER) {
      if (!play) {
        ...
        ballposx = 120;
        ballposy = 350;
        ballxdir = (int)Math.floor(Math.random()*(5-(-5)+1)+(-5));
        ballydir = (int)Math.floor(Math.random()*(5-(-5)+1)+(-5));
        player1 = 310;
        score = 0;
        totalbricks = 21;
        map = new mapbrick(3,7);
